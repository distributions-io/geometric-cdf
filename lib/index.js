...
function cdf( x, options ) {
	...
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.p );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.p, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.p, opts.accessor );
		}
		return cdf2( out, x, opts.p );
	}
	return NaN;
} // end FUNCTION cdf()
